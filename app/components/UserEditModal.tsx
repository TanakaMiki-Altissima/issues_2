...
import { type User, type UserUpdateInput } from '../../lib/mockapi';

interface UserEditModalProps {
  isOpen: boolean;
  user: User | null;
  onClose: () => void;
  onConfirm: (input: UserUpdateInput) => void | Promise<void>;
}
  ...
  return (
    <div className="flex shrink-0 flex-col items-center gap-1 self-center pt-0.5 md:mt-14 lg:self-start">
      <div className="relative">
        <div className="flex h-36 w-36 items-center justify-center rounded-full bg-gray-300" aria-hidden>
          <FontAwesomeIcon icon={faUser} className="text-6xl text-gray-500" />
        </div>
        <div className="absolute bottom-0 right-0 flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white shadow-sm">
          <FontAwesomeIcon icon={faCamera} className="text-lg text-green-600" />
        </div>
      </div>
      <span className="text-lg font-bold leading-none" aria-hidden>
        <FontAwesomeIcon icon={faTriangleExclamation} className="text-lg font-bold leading-none text-red-600" />
      </span>
...
function parseEmailsFromApi(raw?: string): EmailRow[] {
  if (!raw?.trim()) return [{ kind: 'work', value: '' }];
  const rows = raw
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
    .map((s): EmailRow => {
      if (s.startsWith('自宅メール:')) return { kind: 'home', value: s.replace('自宅メール:', '').trim() };
      if (s.startsWith('社用メール:')) return { kind: 'work', value: s.replace('社用メール:', '').trim() };
      return { kind: 'work', value: s };
    });
  return rows.length ? rows : [{ kind: 'work', value: '' }];
}

function parsePhonesFromApi(raw?: string): PhoneRow[] {
  if (!raw?.trim()) return [{ kind: 'mobile', value: '' }];
  const rows = raw
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean)
    .map((s): PhoneRow => {
      if (s.startsWith('自宅:')) return { kind: 'home', value: s.replace('自宅:', '').trim() };
      if (s.startsWith('携帯:')) return { kind: 'mobile', value: s.replace('携帯:', '').trim() };
      return { kind: 'mobile', value: s };
    });
  return rows.length ? rows : [{ kind: 'mobile', value: '' }];
}
...
export function UserEditModal({ isOpen, user, onClose, onConfirm }: UserEditModalProps) {
  ...
  useEffect(() => {
    ...
    setEmails(parseEmailsFromApi(user.email));
    setPhones(parsePhonesFromApi(user.telephone_number));
    setError(null);
  }, [isOpen, user]);
      ...
      <div className="flex min-h-full items-center justify-center p-4 sm:p-6">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl flex max-h-[min(90vh,100dvh-2rem)] flex-col">
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:gap-4">
                <div className="min-w-0 flex-1 space-y-3">
                  <div>
                    ...
                    <input type="text" value={last_name} onChange={(e) => setlast_name(e.target.value)} placeholder="山田" className={inputClass} />
                  </div>
                  <div>
                    <label className="mb-1 block">
                      <RequiredFieldLabel>姓（カナ）</RequiredFieldLabel>
                    </label>
                    <input type="text" value={last_name_okurikana} onChange={(e) => setlast_name_okurikana(e.target.value)} placeholder="ヤマダ" className={inputClass} />
                  </div>
                  <div>
                    <label className="mb-1 block">
                    ...
                    <div className="relative">
                      <input type="text" value={joining_date} onChange={(e) => setjoining_date(e.target.value)} placeholder="年/月/日" className={`${inputClass} pr-10`} />
                      <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-green-700" aria-hidden>
                  ...
                  <div>
                    ...
                    <input type="text" value={id} disabled className={`${inputClass} bg-gray-100 text-gray-500`} />
                  </div>
                </div>
                ...
                <div className="min-w-0 flex-1 space-y-3">
                  <div>
                    ...
                    <input type="text" value={first_name} onChange={(e) => setfirst_name(e.target.value)} placeholder="太郎" className={inputClass} />
                  </div>
                  <div>
                    <label className="mb-1 block">
                      <RequiredFieldLabel>名（カナ）</RequiredFieldLabel>
                    </label>
                    <input type="text" value={first_name_okurikana} onChange={(e) => setfirst_name_okurikana(e.target.value)} placeholder="タロウ" className={inputClass} />
                  </div>
                  <div>
                    <label className="mb-1 block">
            ...
            <SectionTile title="所属情報" bodyClassName="">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <input type="text" value={department} onChange={(e) => setdepartment(e.target.value)} placeholder="会社・所属" className={inputClass} />
                </div>
                <div className="min-w-0 flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">部署</label>
                  <input type="text" value={affiliation} onChange={(e) => setaffiliation(e.target.value)} placeholder="部署" className={inputClass} />
                </div>
                <div className="min-w-0 flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">役職/階級</label>
                  <input type="text" value={position} onChange={(e) => setposition(e.target.value)} placeholder="役職/階級" className={inputClass} />
                </div>
              </div>
            </SectionTile>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setEmails((prev) => [...prev, { kind: 'work', value: '' }])}
                  ...
                  {emails.slice(1).map((row, i) => (
                    <div key={i + 1} className={comboOuterClass}>
                      <select
                        ...
                        onChange={(e) => {
                          const kind = e.target.value as EmailRow['kind'];
                          setEmails((prev) => prev.map((x, j) => (j === i + 1 ? { ...x, kind } : x)));
                        }}
                        className={comboSelectClass}
                        aria-label="メールの種類"
                      >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          setEmails((prev) => prev.map((x, j) => (j === i + 1 ? { ...x, value: v } : x)));
                        }}
                        placeholder="name@example.com"
                        className={comboInputClass}
                      />
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setPhones((prev) => [...prev, { kind: 'mobile', value: '' }])}
                      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-600 text-lg font-medium leading-none text-white shadow-sm hover:bg-green-700"
                      aria-label="電話番号を追加"
                    >
                      +
                  ...
                  {phones.slice(1).map((row, i) => (
                    <div key={i + 1} className={comboOuterClass}>
                      <select
                        ...
                        onChange={(e) => {
                          const kind = e.target.value as PhoneRow['kind'];
                          setPhones((prev) => prev.map((x, j) => (j === i + 1 ? { ...x, kind } : x)));
                        }}
                        className={comboSelectClass}
                        aria-label="電話の種類"
                      >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          setPhones((prev) => prev.map((x, j) => (j === i + 1 ? { ...x, value: v } : x)));
                        }}
                        placeholder="090-1234-5678"
                        className={comboInputClass}
                      />
          ...
          <div className="flex justify-end gap-2 px-6 pb-6 pt-2 flex-shrink-0 border-t border-gray-200 bg-white rounded-b-lg">
            <button type="button" onClick={handleBack} className="bg-gray-200 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-300">
              キャンセル
            </button>
            <button
              type="button"
